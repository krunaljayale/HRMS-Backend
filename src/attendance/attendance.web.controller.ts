...
export class AttendanceWebController {
  ...
  async checkIn(
    ...
    const data = await this.attendanceService.checkIn({ employeeId }, body);
  ...
  async checkOut(
    ...
    // Pass the payload and the checkout details to the service
    const data = await this.attendanceService.checkOut({ employeeId }, body);
