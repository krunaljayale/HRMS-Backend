...
export class AttendanceService {
    ...
    private async validateLocation(
        ...
        workMode: string,
        employeeIdentifier?: string, // Name or Employee Code
    ) {
        if (workMode === 'Office') {
            if (latitude == null || longitude == null) {
                console.error(`[LOCATION ALERT] Punch blocked: Missing coordinates for ${employeeIdentifier || 'Unknown Employee'}`);
            ...
            if (distance > config.radius_meters) {
                // 🔍 Clear, actionable log for you to trace in Lightsail / PM2 logs
                console.error(
                    `🚨 [LOCATION REJECTED] Employee: ${employeeIdentifier || 'Unknown'} | ` +
                    `Distance: ${Math.round(distance)}m away (Max allowed: ${config.radius_meters}m) | ` +
                    `Punched Lat/Lon: (${latitude}, ${longitude})`
                );
    ...
    async checkIn(jwtPayload: any, dto: CheckInDto) {
        ...
        try {
            ...
            await this.validateLocation(
                ...
                dto.longitude,
                dto.workMode,
                `${fullEmployee.name} (${fullEmployee.employeeCode})`
            );
    ...
    async checkOut(jwtPayload: any, dto: CheckOutDto) {
        ...
        try {
            ...
            const fullEmployee = await this.employeeService.getEmployeeById(jwtPayload.employeeId, 'employeeCode name');
