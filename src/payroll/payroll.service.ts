...
export class PayrollService {
  ...
  private async calculatePayrollMetrics(
    ...
    // Fetch Data Concurrently using the BUFFERS + Reimbursements
    const [attendances, holidays, approvedLeaves, unpaidClaims] = await Promise.all([
      this.attendanceService.findRecordsInRange(employeeId, bufferFrom, bufferTo),
      this.holidayService.findHolidaysInRange(bufferFrom, bufferTo),
      this.leaveService.findApprovedLeavesInRange(employeeId, bufferFrom, bufferTo),
      this.reimbursementService.getApprovedUnpaidClaims(employeeId, toDate),
    ]);
  ...
  async previewEmployeePayroll(
    ...
    // --- ENRICHMENT STEP ---
    // Using a ternary operator directly bypasses TypeScript's never[] inference
